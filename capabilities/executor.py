...
class CommandExecutor(QObject):
    ...
    def get_video_transcript(self, query: str) -> str:
        """Extract auto-generated captions/transcript for a YouTube video."""
        return self.youtube_service.get_video_transcript(query)

    def get_trending_videos(self, region: str = "US", count: int = 5) -> str:
        """Retrieve currently trending videos on YouTube."""
        return self.youtube_service.get_trending_videos(region, count)

    def get_channel_videos(self, channel_name: str, count: int = 5) -> str:
        """List recent uploads from a YouTube channel."""
        return self.youtube_service.get_channel_videos(channel_name, count)

    def play_youtube_playlist(self, url: str, max_videos: int = 10) -> str:
        """Load a YouTube playlist into the queue and play the first video."""
        return self.youtube_service.play_youtube_playlist(url, max_videos)
