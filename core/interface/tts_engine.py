...
class TTSEngine:
    ...
    def __init__(self, config):
        ...
        # === AUDIO PHRASE CACHE (LRU, max 50 entries) ===
        # Caches generated PCM arrays for repeated short phrases so
        # acknowledgment and progress phrases never re-synthesize.
    ...
    def _pre_cache_phrases(self):
        """Pre-generate audio for the most common short phrases used by
        ThinkingFeedback so they play instantly on first use."""
        common_phrases = [
            # Generic acks
            "On it!", "Got it!", "Sure thing!", "Right away!",
            # Media
            "Pulling that up!", "Loading it now!", "Tuning in!",
            # Search
            "Searching now!", "Looking that up for you!",
            # System
            "Opening that now!", "Starting it up!",
            # Download
            "Starting the download!", "Grabbing that for you!",
        ]
        for phrase in common_phrases:
            cache_key = (phrase.lower().strip(), 1.0)
            with self._audio_cache_lock:
                if cache_key in self._audio_cache:
                    continue
            try:
                audio_data, _ = self.kokoro.create(phrase, voice=self.voice_name, speed=1.0)
                with self._audio_cache_lock:
                    self._audio_cache[cache_key] = audio_data
                    if len(self._audio_cache) > self._audio_cache_max:
                        self._audio_cache.popitem(last=False)  # evict oldest
                logger.debug(f"📦 Pre-cached phrase: '{phrase}'")
            except Exception as e:
                logger.debug(f"Phrase pre-cache skipped '{phrase}': {e}")
        logger.info(f"✅ TTS phrase pre-cache loaded ({len(self._audio_cache)} entries)")
    ...
    def _init_kokoro(self):
        ...
        try:
            ...
            # === ONNX WARMUP PASS ===
            # The first ONNX inference is slow (~1-2 s extra) due to kernel JIT.
            # Running a silent dummy inference here eliminates that cold-start
            # penalty from the first real user-facing phrase.
            try:
                warmup_start = time.time()
                logger.info("🔥 Running ONNX warmup pass...")
                _dummy, _ = self.kokoro.create("Ready.", voice=self.voice_name, speed=1.0)
                warmup_time = time.time() - warmup_start
                logger.info(f"✅ ONNX warmup done in {warmup_time:.2f}s")
                # Pre-cache the warmup phrase so it's instantly available
                self._audio_cache[("ready.", 1.0)] = _dummy
            except Exception as e:
                logger.warning(f"⚠️ ONNX warmup failed (non-fatal): {e}")

            # Pre-cache common acknowledgment phrases in background thread
            threading.Thread(
                target=self._pre_cache_phrases,
                daemon=True,
                name="TTS-PhrasePreCache",
            ).start()
    ...
    def _generate_audio(self, sentence: str, speed: float):
        """Generate audio for a single sentence, with LRU cache for short phrases.

        Returns:
            numpy array of audio data, or None on error
        """
        # Normalise key (cache is case-insensitive, speed-sensitive)
        cache_key = (sentence.lower().strip(), speed)

        # Check cache first (lock-safe)
        with self._audio_cache_lock:
            if cache_key in self._audio_cache:
                # Move to end (most-recently-used)
                self._audio_cache.move_to_end(cache_key)
                logger.debug(f"🎵 Cache HIT for phrase: '{sentence[:40]}'")
        ...
        with self._audio_cache_lock:
            self._audio_cache[cache_key] = audio_data
            if len(self._audio_cache) > self._audio_cache_max:
                self._audio_cache.popitem(last=False)  # evict LRU entry
