...
class ContentBoxFormatter:
    ...
    def create_formatting_toolbar(self) -> QWidget:
        ...
        italic_font = QFont("Segoe UI", 10, QFont.Weight.Normal)
    ...
    # ========== TOOLBAR STATE SYNC (MS Word-like) ==========
    
    def sync_toolbar_state(self):
        """
        Sync toolbar buttons/combos with the formatting at the current cursor position.
        Called whenever the cursor moves or selection changes — like MS Word.
        """
        cursor = self.text_editor.textCursor()
        char_fmt = cursor.charFormat()
        block_fmt = cursor.blockFormat()
        
        # --- Bold / Italic / Underline button toggle ---
        if self.bold_btn:
            is_bold = char_fmt.fontWeight() >= QFont.Weight.Bold
            self._set_button_checked(self.bold_btn, is_bold)
        
        if self.italic_btn:
            self._set_button_checked(self.italic_btn, char_fmt.fontItalic())
        
        if self.underline_btn:
            self._set_button_checked(self.underline_btn, char_fmt.fontUnderline())
        
        # --- Font family combo ---
        if self.font_combo:
            font_family = char_fmt.fontFamily() or self.current_font
            # Block signal to avoid recursive formatting
        ...
        # --- Font size combo ---
        if self.size_combo:
            size = int(char_fmt.fontPointSize()) if char_fmt.fontPointSize() > 0 else self.current_size
            self.size_combo.blockSignals(True)
            idx = self.size_combo.findText(str(size))
            if idx >= 0:
        ...
        if self.align_left_btn:
            self._set_button_checked(self.align_left_btn,
                                     alignment == Qt.AlignmentFlag.AlignLeft or alignment == Qt.AlignmentFlag(0))
        if self.align_center_btn:
            self._set_button_checked(self.align_center_btn,
                                     alignment == Qt.AlignmentFlag.AlignHCenter)
        if self.align_right_btn:
            self._set_button_checked(self.align_right_btn,
                                     alignment == Qt.AlignmentFlag.AlignRight)
        if self.align_justify_btn:
            self._set_button_checked(self.align_justify_btn,
                                     alignment == Qt.AlignmentFlag.AlignJustify)
    
    @staticmethod
    def _set_button_checked(btn: QPushButton, checked: bool):
        """
        Visually toggle a toolbar button's checked/active state.
        Adds or removes a cyan border/highlight to indicate active formatting.
        Preserves existing stylesheet by appending/removing the active rule.
        """
        # Get current stylesheet, strip any previous active overlay
        base = btn.styleSheet()
        # Remove previous injected active marker if present
        marker = "/* ACTIVE_FMT */"
        if marker in base:
        ...
        if checked:
            active_style = (
                f"\n{marker}\n"
                "QPushButton { border: 2px solid #00D4FF; "
                "background: rgba(0, 212, 255, 60); }"
            )
            btn.setStyleSheet(base + active_style)
        else:
